...
// Inject minimal, scoped spacing fixes for QR/PR forms (no global side-effects)
function ensureMintFlowFormSpacing(){
  if(document.getElementById("mf-form-spacing")) return;
  const style = document.createElement("style");
  style.id = "mf-form-spacing";
  style.textContent = `
    /* TOP FORM: label close to its own input, but rows spaced */
    #frmCreate .field, #frmCreatePR .field { margin-bottom: 16px; }
    #frmCreate .field > label, #frmCreatePR .field > label { display:block; margin:0 0 6px 0 !important; line-height:1.2; }
    #frmCreate .field > label .sub, #frmCreatePR .field > label .sub { display:block; margin-top:2px; font-weight:300; opacity:.65; }
    #frmCreate .row, #frmCreatePR .row { gap: 12px; align-items:flex-start; }

    /* ITEMS: same label/input rhythm, no "stack collision" */
    #items .field, #prItems .field { margin-bottom: 14px; }
    #items .field > label, #prItems .field > label { margin:0 0 6px 0 !important; }
    #items .row, #prItems .row { gap: 12px; align-items:flex-start; }

    /* Export by + Remark: keep them close (no football field gap) */
    .mf-export-row { display:flex; gap:12px; align-items:center; flex-wrap:wrap; }
    .mf-export-row .mf-remark { flex: 1 1 260px; min-width: 220px; }
  `;
  ...
  const addItem = ()=>{
    ...
    block.innerHTML = `
      ...
      <div class="row">
        ...
        <div class="field">
          <label>${biLabel("Unit", "หน่วย")} <span class="sub">+ เพิ่มเองได้</span></label>
<input class="input" name="unit" list="mfUnitList" />

<datalist id="mfUnitList">
  ...
  <label>${biLabel("Export By.", "ส่งออกโดย")}</label>
  <div class="mf-export-row">
    <label style="display:flex;gap:6px;align-items:center;margin:0"><input type="checkbox" name="exportBy" value="By Sea"> By Sea</label>
    <label style="display:flex;gap:6px;align-items:center;margin:0"><input type="checkbox" name="exportBy" value="By Land"> By Land</label>
    <label style="display:flex;gap:6px;align-items:center;margin:0"><input type="checkbox" name="exportBy" value="By Air"> By Air</label>
  </div>
</div>
<div class="field mf-remark" style="flex:1">
