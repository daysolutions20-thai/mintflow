...
<div id="items"></div>
  ...
  const ensureUnitList = () => {
    ...
    // Make unit list available on both PR + QR (create if missing)
    if(!dl){
...
const fillUnitSelect = (sel) => {
  if(!sel) return;
  ensureUnitList();
  const dl = document.getElementById("unitList");
  const cur = sel.value;
  // Rebuild options from datalist (shared)
  sel.innerHTML = '<option value="">-- Select unit --</option>';
  Array.from((dl ? dl.querySelectorAll("option") : [])).forEach(o => {
    ...
    block.innerHTML = `
      ...
      <div class="row row-codeqty">
        ...
        <div class="field">
          ...
          <select class="input" name="unit" required></select>
        </div>
      </div>
...
// Unit (dropdown, shared list; no "+")
const unitSel = block.querySelector('select[name="unit"]');
if(unitSel) fillUnitSelect(unitSel);
